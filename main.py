...
async def start_calibrate(req: dict = None):
    ...
    Reads optional knobs from the JSON body:
      - time_budget_sec (default 240)
      - target_ss_rate (default 0.80)
      - target_mean (default 17500)
      - ss_threshold (default 17500)
      - sims_per_candidate (default 3)
      - baseline_sims (default 4)
    ...
    cmd_args = [
        py_exe,
        str(script_path),
        "--time-budget-sec", str(float(body.get("time_budget_sec") or 240.0)),
        "--target-ss-rate", str(float(body.get("target_ss_rate") or 0.80)),
        ...
        "--sims-per-candidate", str(int(body.get("sims_per_candidate") or 3)),
        "--baseline-sims", str(int(body.get("baseline_sims") or 4)),
...
def _apply_cached_deck_policy(preset: dict):
    ...
    The cache is built offline by `tools/optimize_deck_policy.py`. User-set
    `learned_hyperparameters` keys WIN — the cache only fills gaps. This
    keeps any manual overrides intact while pulling in the offline-tuned
    values for parameters the user hasn't touched.
    """
    try:
    ...
    added = {k: v for k, v in (preset.get("learned_hyperparameters") or {}).items() if k not in before}
    print(f"deck policy: applied cached entry {signature} "
          f"(added {len(added)} hyperparameter keys: {list(added.keys())})",
          flush=True)
